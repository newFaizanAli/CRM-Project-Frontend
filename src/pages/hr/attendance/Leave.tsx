...
import LeaveForm from "../../../components/form/hr/LeaveForm";
...
const LeavePage = () => {
  const { leaves, fetchLeaves, deleteLeave, isFetched } = useLeaveStore();
  ...
  }, [fetchLeaves, isFetched]);

  const table = useReactTable({
    data: isFiltered ? filteredData : leaves,
    columns,
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg max-h-screen overflow-y-auto w-full max-w-3xl p-6">
            ...
            <LeaveForm leave={selectedLeave} onClose={handleClose} />
          </div>
        </div>
      )}
